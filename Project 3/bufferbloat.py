...
def start_iperf(net):
    ...
    # long lived TCP flow.
    h1 = net.get('h1')
    h1.popen("iperf -c %s -t 999999999 > %s/iperf.out" % (h2.IP(), args.dir), shell=True)
...
def bufferbloat():
    ...
    while True:
        ...
        for _ in range(0, 3):
            valid = client.popen("curl -ILs %s/http/index.html ^HTTP" %(server.IP()) , shell=True).communicate()[0]
            # 200 ---> request success
            if "200".encode(encoding = "utf-8") in valid:   
                print("pass sanity")   
                response_t = client.popen("curl -o /dev/null -s -w %%{time_total} %s/http/index.html" % server.IP(), shell=True).communicate()[0]
